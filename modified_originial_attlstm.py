...
class DA_RNN(nn.Module):
    ...
    def test(self,X_test,y_test, on_train=False):
        """Prediction."""

        if on_train:
            y_pred = np.zeros(self.train_timesteps - self.T + 1)
        else:
            y_pred = np.zeros(X_test.shape[0]) #self.X.shape[0] - self.train_timesteps
        ...
        while i < len(y_pred):
            ...
            for j in range(len(batch_idx)):
                if on_train:
                    ...
                    X[j, :, :] = X_test[batch_idx[j]:( batch_idx[j] + self.T - 1), :]
                    y_history[j, :] = y_test[batch_idx[j]:( batch_idx[j] + self.T - 1)]
